...
static PyObject *generalized_intersect(PyObject *self, PyObject *args) {
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if (!PyLong_CheckExact(tokenObj)) {
      PyErr_SetString(PyExc_TypeError, "Token[0] is not a int");
      return NULL;
    }
  ...
  std::vector< std::shared_ptr<NegatableIterator<MathyRow>> > iters;
  for (std::pair<uint64_t, bool> token : tokens) {
    iters.push_back(std::make_shared<NegatableIterator<MathyRow>>(index->iterator(
      token.first,
      MathyRow{
        (uint32_t)docid,
        (uint32_t)value,
  ...
  GeneralIntersectionIterator<MathyRow> it(iters);
